...
def test_request_timeout_key():
    assert load_settings(env={})["timeout"] == 12
...
def test_nested_auth_token():
    assert load_settings(env={})["api_key"] == "local-token-v2"


def test_missing_required_api_key_fails_loudly(tmp_path):
    config = write_config(tmp_path / "config.json", {"request_timeout": 3, "region": "us-west-2"})
...
def test_request_timeout_type_is_int(tmp_path):
    config = write_config(
        ...
        {"request_timeout": "15", "auth": {"token": "t"}, "region": "us-west-2"},
    )
